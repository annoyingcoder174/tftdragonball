...
async function loadAugmentStats() {
    try {
        const snapshot = await firebase.firestore().collection("augmentStats").get();

        snapshot.forEach(doc => {
            const data = doc.data();
            const augmentName = doc.id;
            const augment = findAugmentByName(augmentName);

            if (!augment) return;

            const winRate = data.total > 0 ? ((data.win / data.total) * 100).toFixed(2) : 0;
            ...
            allAugments.push({
                ...
                win: data.win,
                type: "augment",
                tier: getTierByName(augment.name),
                tierClass
            });
        });
        ...
        console.error("Error loading augment stats:", error);
    }
}
...
async function loadChampStats() {
    try {
        ...
        snapshot.forEach(doc => {
            const data = doc.data();
            const champName = doc.id;
            const champ = findChampionByName(champName);
        ...
        console.error("Error loading champ stats:", error);
    }
}
...
function applyFilters() {
    ...
    // Combine all data
    const allStats = [...allAugments, ...allChamps];
    ...
    const filteredStats = allStats
        ...
        .sort((a, b) => b.winRate - a.winRate); // Always sort by highest to lowest winrate
...
function createCard(data) {
    ...
    card.className = `stat-card ${data.tierClass}`;
    ...
    winRateDiv.className = "stat-desc";
    winRateDiv.textContent = `Tỉ lệ thắng: ${data.winRate}% (${data.win} / ${data.total})`;
...
function getTierClass(winRate) {
    if (winRate >= 90) return "stat-tier-z";
    if (winRate >= 80) return "stat-tier-s";
    if (winRate >= 60) return "stat-tier-a";
    if (winRate >= 50) return "stat-tier-b";
    if (winRate >= 30) return "stat-tier-c";
    return "stat-tier-d";
}

function getTierByName(name) {
    for (const tier of Object.keys(augments)) {
        if (augments[tier].some(aug => aug.name === name)) {
            return tier;
        }
    }
    return "unknown";
...
function findAugmentByName(name) {
    return Object.values(augments).flat().find(aug => aug.name === name);
}

function findChampionByName(name) {
    return Object.values(champions).flat().find(champ => champ.name === name);
}
const champions = {
    D: [
